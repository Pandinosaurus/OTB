...
bool PatternSampler::ParameterType::operator!=(const PatternSampler::ParameterType& param) const
{
...
void PatternSampler::Reset(void)
{
  Superclass::Reset();
  m_Index1 = 0UL;
  m_Index2 = 0UL;
...
bool PatternSampler::TakeSample(void)
{
  bool ret = false;
...
// [static]
void PatternSampler::ImportPatterns(const std::string& data, ParameterType& param)
{
...
// [static]
void PatternSampler::ExportPatterns(const ParameterType& param, std::string& data)
{
  // clear output string
  data.clear();
...
unsigned int PatternSampler::ParseSymbol(const char& s)
{
  if ((s == '1') || (s == 'X') || (s == 'y') || (s == 'Y') || (s == '|') || (s == '+'))
  {
    return 1;
  }
  else if ((s == '0') || (s == '_') || (s == 'n') || (s == 'N') || (s == '.') || (s == '-'))
  {
    return 0;
  }
  return 2;
...
std::vector<bool> PatternSampler::RandArray(unsigned long N, unsigned long T)
{
  if (N > T)
...
unsigned long PatternSampler::FindBestSize(unsigned long tot)
{
  // handle small values
