...
#define otbCheckStringValue(Comment, RefStr, TestStr, count, report) \
  {                                                                  \
    std::string strRefStr("");                                       \
    if (RefStr != NULL)                                              \
      strRefStr = RefStr;                                            \
    std::string strTestStr("");                                      \
    if (TestStr != NULL)                                             \
      strTestStr = TestStr;                                          \
...
int TestHelper::RegressionTestAllImages(const StringList& baselineFilenamesImage, const StringList& testFilenamesImage, const EpsilonList& epsilons)
{
  int           result              = 0;
  ...
  for (; (itbaselineFilenames != baselineFilenamesImage.end()) && (itTestFilenames != testFilenamesImage.end()) && (itEpsilon != epsilons.end());
       ++itbaselineFilenames, ++itTestFilenames, ++itEpsilon)
  {
    ...
    while (baseline != baselines.rend() && (multiResult != 0))
    {
      ...
      baseline->second = this->RegressionTestImage(cpt, testFilenameImage.c_str(), (baseline->first).c_str(), *itEpsilon);
...
int TestHelper::RegressionTestAllMetaData(const StringList& baselineFilenamesMetaData, const StringList& testFilenamesMetaData, const EpsilonList& epsilons)
{
  int result = 0;
  ...
  EpsilonListIt itEpsilon           = epsilons.begin();
  // For each couple of baseline and test file, do the comparison
  for (; (itbaselineFilenames != baselineFilenamesMetaData.end()) && (itTestFilenames != testFilenamesMetaData.end()) && (itEpsilon != epsilons.end());
       ++itbaselineFilenames, ++itTestFilenames, ++itEpsilon)
  {
    std::string baselineFilenameImage = (*itbaselineFilenames);
    std::string testFilenameImage     = (*itTestFilenames);
    ...
    while (baseline != baselines.rend() && (multiResult != 0))
    {
      std::cout << "Testing non-regression on image: " << (baseline->first) << std::endl;
      this->ReportErrorsOff();
      baseline->second = this->RegressionTestMetaData(testFilenameImage.c_str(), (baseline->first).c_str(), *itEpsilon);
...
int TestHelper::RegressionTestAllAscii(const StringList& baselineFilenamesAscii, const StringList& testFilenamesAscii, const EpsilonList& epsilons,
                                       const StringList& ignoredLines)
{
  ...
  // For each couple of baseline and test file, do the comparison
  for (; (itbaselineFilenames != baselineFilenamesAscii.end()) && (itTestFilenames != testFilenamesAscii.end()) && (itEpsilon != epsilons.end());
       ++itbaselineFilenames, ++itTestFilenames, ++itEpsilon)
  {
    ...
    while (baseline != baselines.rend() && (multiResult != 0))
    {
      ...
      this->ReportErrorsOff();
      baseline->second = this->RegressionTestAsciiFile(testFilenameAscii.c_str(), (baseline->first).c_str(), *itEpsilon, ignoredLines);
...
int TestHelper::RegressionTestAllDiff(const StringList& baselineFilenamesAscii, const StringList& testFilenamesAscii, const EpsilonList& epsilons,
                                      const StringList& ignoredLines)
{
  int result = 0;
  // Creates iterators on baseline filenames vector and test filenames vector
       ...
       ++itbaselineFilenames, ++itTestFilenames, ++itEpsilon)
  {
    ...
    while (baseline != baselines.rend() && (multiResult != 0))
    {
      std::cout << "Testing non-regression on file: " << (baseline->first) << std::endl;
      this->ReportErrorsOff();
      baseline->second = this->RegressionTestDiffFile(testFilenameAscii.c_str(), (baseline->first).c_str(), *itEpsilon, ignoredLines);
...
int TestHelper::RegressionTestAllBinary(const StringList& baselineFilenamesBinary, const StringList& testFilenamesBinary)
{
  int result = 0;
  // Creates iterators on baseline filenames vector and test filenames vector
  ...
  // For each couple of baseline and test file, do the comparison
  for (; (itbaselineFilenames != baselineFilenamesBinary.end()) && (itTestFilenames != testFilenamesBinary.end()); ++itbaselineFilenames, ++itTestFilenames)
  {
...
int TestHelper::RegressionTestAllOgr(const StringList& baselineFilenamesOgr, const StringList& testFilenamesOgr, const EpsilonList& epsilons)
{
  int result = 0;
  // Creates iterators on baseline filenames vector and test filenames vector
  ...
  EpsilonListIt itEpsilon           = epsilons.begin();
  // For each couple of baseline and test file, do the comparison
  for (; (itbaselineFilenames != baselineFilenamesOgr.end()) && (itTestFilenames != testFilenamesOgr.end()) && (itEpsilon != epsilons.end());
       ++itbaselineFilenames, ++itTestFilenames, ++itEpsilon)
  {
    std::string baselineFilenameOgr = (*itbaselineFilenames);
    std::string testFilenameOgr     = (*itTestFilenames);

    std::map<std::string, int>                   baselines = this->RegressionTestBaselines(const_cast<char*>(baselineFilenameOgr.c_str()));
    std::map<std::string, int>::reverse_iterator baseline  = baselines.rbegin();
    int multiResult = 1;
    ...
    while (baseline != baselines.rend() && (multiResult != 0))
    {
      ...
      this->ReportErrorsOff();
      baseline->second = this->RegressionTestOgrFile(testFilenameOgr.c_str(), (baseline->first).c_str(), *itEpsilon);
...
int TestHelper::RegressionTestAsciiFile(const char* testAsciiFileName, const char* baselineAsciiFileName, const double epsilon,
                                        std::vector<std::string> ignoredLines) const
{
  ...
  while ((itRef != listLineFileRef.end()) && (itTest != listLineFileTest.end()))
  {
    ...
    if (ignoredLines.size() > 0)
    {
      for (std::vector<std::string>::iterator itIgnoredLines = ignoredLines.begin(); itIgnoredLines != ignoredLines.end(); ++itIgnoredLines)
      {
    ...
    if ((!ignoreCurrentLineRef) && (!ignoreCurrentLineTest))
    {
      differenceFoundInCurrentLine =
          CompareLines(strfileref, strfiletest, nbdiff, fluxfilediff, numLine, listStrDiffLineFileRef, listStrDiffLineFileTest, epsilon);
    ...
    if (m_IgnoreLineOrder)
    {
      if ((!ignoreCurrentLineRef) && (!ignoreCurrentLineTest))
      {
        if (differenceFoundInCurrentLine)
        {
          if (*itRef > *itTest)
          {
            fluxfilediff << "Additional line in test file: "
                         << " : " << *itTest << std::endl;
            ++itTest;
          }
          else
          {
            fluxfilediff << "Additional line in ref file: "
                         << " : " << *itTest << std::endl;
            ++itRef;
          }
        }
...
int TestHelper::RegressionTestDiffFile(const char* testAsciiFileName, const char* baselineAsciiFileName, const double epsilon, StringList ignoredLines) const
{
  ...
  for (posRef = 0; posRef < listLineFileRef.size(); posRef++)
  {
    ...
    for (curPosTest = posTest; curPosTest < listLineFileTest.size(); curPosTest++)
    {
      ...
      for (unsigned int i = 0; i < std::min(nbTokenRef, nbTokenTest); i++)
      {
        ...
        if (curLineRef.compare(tokenRef[i].begin() - curLineRef.begin(), tokenRef[i].size(), curLineTest, tokenTest[i].begin() - curLineTest.begin(),
                               tokenTest[i].size()) == 0)
        {
        ...
        else
        {
          ...
          bool isRefTokenHexa  = isHexaPointerAddress(curLineRef, tokenRef[i].begin() - curLineRef.begin(), tokenRef[i].size());
          bool isTestTokenHexa = isHexaPointerAddress(curLineTest, tokenTest[i].begin() - curLineTest.begin(), tokenTest[i].size());
          ...
          else
          {
            ...
            else
            {
              ...
              {
                if (curLineRef.compare(tokenRef[i].begin() - curLineRef.begin(), tokenRef[i].size(), m_SpecialTokens[j].first) == 0 &&
                    curLineTest.compare(tokenTest[i].begin() - curLineTest.begin(), tokenTest[i].size(), m_SpecialTokens[j].second) == 0)
                {
                  areTokensEquivalent = true;
                  break;
                }
    ...
    else
    {
      ...
      if (bestCommonTokens < static_cast<int>(std::max(nbTokenRef, nbTokenTestSelected)))
      {
        ...
        for (unsigned int k = 0; k < std::min(nbTokenRef, nbTokenTestSelected); k++)
        {
          ...
          {
            ossBase << curLineRef.substr(tokenRef[k].begin() - curLineRef.begin(), tokenRef[k].size());
            ossTest << std::string(tokenRef[k].size(), '_');
          }
          else
          {
            unsigned int maxTokenSize = std::max(tokenRef[k].size(), tokenTestSelected[k].size());
            ossBase << curLineRef.substr(tokenRef[k].begin() - curLineRef.begin(), tokenRef[k].size()) << std::string(maxTokenSize - tokenRef[k].size(), ' ');
            ossTest << lineTestSelected.substr(tokenTestSelected[k].begin() - lineTestSelected.begin(), tokenTestSelected[k].size())
...
int TestHelper::TokenizeLine(const std::string& line, StringList& tokens) const
{
  ...
  for (pos = 0; pos < line.size(); pos++)
  {
    ...
    if (isAlphaNum(curChar) || isPoint(curChar) || isMinusSign(curChar) || curChar == 94 || curChar == 95 || curChar == 47)
    {
      isSeparator = false;
    }
...
int TestHelper::RegressionTestMetaData(const char* testImageFilename, const char* baselineImageFilename, const double tolerance) const
{
  ...
  if (blImPtr->GetProjectionRef().compare(testImPtr->GetProjectionRef()) != 0)
  {
    ...
    std::cerr << "baseline image: " << baselineImageFilename << " has projection reference " << blImPtr->GetProjectionRef() << std::endl;
    std::cerr << "Test image:     " << testImageFilename << " has projection reference " << testImPtr->GetProjectionRef() << std::endl;
    errcount++;
  }
  ...
  if (blImPtr->GetGeoTransform() != testImPtr->GetGeoTransform())
  {
    ...
    std::cerr << "baseline image: " << baselineImageFilename << " has geographic transform " << VectorToString(blImPtr->GetGeoTransform()) << std::endl;
    std::cerr << "Test image:     " << testImageFilename << " has geographic transform " << VectorToString(testImPtr->GetGeoTransform()) << std::endl;
    errcount++;
  }
  ...
  if (blImPtr->GetUpperLeftCorner() != testImPtr->GetUpperLeftCorner())
  {
    ...
    std::cerr << "baseline image: " << baselineImageFilename << " has upper left corner " << VectorToString(blImPtr->GetUpperLeftCorner()) << std::endl;
    std::cerr << "Test image:     " << testImageFilename << " has upper left corner " << VectorToString(testImPtr->GetUpperLeftCorner()) << std::endl;
    errcount++;
  }
  ...
  if (blImPtr->GetUpperRightCorner() != testImPtr->GetUpperRightCorner())
  {
    ...
    std::cerr << "baseline image: " << baselineImageFilename << " has upper right corner " << VectorToString(blImPtr->GetUpperRightCorner()) << std::endl;
    std::cerr << "Test image:     " << testImageFilename << " has upper right corner " << VectorToString(testImPtr->GetUpperRightCorner()) << std::endl;
    errcount++;
  }
  ...
  if (blImPtr->GetLowerLeftCorner() != testImPtr->GetLowerLeftCorner())
  {
    ...
    std::cerr << "baseline image: " << baselineImageFilename << " has lower left corner " << VectorToString(blImPtr->GetLowerLeftCorner()) << std::endl;
    std::cerr << "Test image:     " << testImageFilename << " has lower left corner " << VectorToString(testImPtr->GetLowerLeftCorner()) << std::endl;
    errcount++;
  }
  ...
  if (blImPtr->GetLowerRightCorner() != testImPtr->GetLowerRightCorner())
  {
    ...
    std::cerr << "baseline image: " << baselineImageFilename << " has lower right corner " << VectorToString(blImPtr->GetLowerRightCorner()) << std::endl;
    std::cerr << "Test image:     " << testImageFilename << " has lower right corner " << VectorToString(testImPtr->GetLowerRightCorner()) << std::endl;
    errcount++;
  }
...
std::map<std::string, int> TestHelper::RegressionTestBaselines(char* baselineFilename) const
{
  ...
  for (unsigned int i = 0; i < nbFiles; i++)
  {
    ...
    if ((curFilename.substr(0, sizeRef) == originalBaseline) && reg.find(curFilename.substr(sizeRef, curFilename.size())) &&
...
int TestHelper::RegressionTestOgrFile(const char* testOgrFilename, const char* baselineOgrFilename, const double toleranceDiffValue) const
{
  ...
  if (ref_poDS == nullptr)
  {
    ...
    {
      std::cout << "FAILURE:\n"
                   "Unable to open REF datasource `"
                << ref_pszDataSource << "' with the following drivers." << std::endl;
  ...
  if (test_poDS == nullptr)
  {
    ...
    {
      std::cout << "FAILURE:\n"
                   "Unable to open TEST datasource `"
                << test_pszDataSource << "' with the following drivers." << std::endl;
  ...
  for (int iLayer = 0; iLayer < ref_poDS->GetLayerCount(); ++iLayer)
  {
    ...
    if (nbdiff == 0)
    {
      ...
      while ((ref_poFeature = ref_poLayer->GetNextFeature()) != nullptr)
      {
        ...
        ignoredLines.clear();
        nbdiff = RegressionTestAsciiFile(test_filename.c_str(), ref_filename.c_str(), toleranceDiffValue, ignoredLines);
...
void TestHelper::DumpOGRFeature(FILE* fpOut, OGRFeature* feature, char** papszOptions)
{
  ...
  const char* pszDisplayFields = CSLFetchNameValue(papszOptions, "DISPLAY_FIELDS");
  if (pszDisplayFields == nullptr || CSLTestBoolean(pszDisplayFields))
  {
    for (int iField = 0; iField < feature->GetFieldCount(); iField++)
    {
      ...
      if (ogr::IsFieldSetAndNotNull(feature, iField))
  ...
  {
    const char* pszDisplayStyle = CSLFetchNameValue(papszOptions, "DISPLAY_STYLE");
...
void TestHelper::DumpOGRGeometry(FILE* fp, OGRGeometry* geometry, const char* pszPrefix, char** papszOptions)
{
    ...
    case wkbPolygon25D:
    {
      ...
      if (nRings)
      {
        ...
        for (ir = 0; ir < nRings; ir++)
        {
          if (ir)
            fprintf(fp, ", ");
          fprintf(fp, "%d points", poPoly->getInteriorRing(ir)->getNumPoints());
        }
        fprintf(fp, ")");
      }
      fprintf(fp, "\n");
  ...
  else if (pszDisplayGeometry == nullptr || CSLTestBoolean(pszDisplayGeometry) || EQUAL(pszDisplayGeometry, "WKT"))
  {
...
{
  return (((i > 47) && (i < 58)) || ((i > 96) && (i < 103)) || ((i > 64) && (i < 71)));
}
...
{
  if (str.find("RTTI") != std::string::npos)
    return true;
  if (str.find("Modified Time") != std::string::npos)
    return true;
  if (str.find("PipelineMTime:") != std::string::npos)
    return true;
  return false;
}
...
bool TestHelper::isHexaPointerAddress(const std::string& str, size_t pos, size_t size) const
{
  ...
  if (!(str[pos + 1] == 120 || (str[pos + 1] == 48 && size == 16) || (size == 8)))
  {
    return false;
  }
...
bool TestHelper::CompareLines(const std::string& strfileref, const std::string& strfiletest, int& nbdiff, std::ofstream& fluxfilediff, int& numLine,
    ...
    if (!isHexaPointerAddress(strRef))
    {
      ...
      if (isScientificNumeric(strRef))
      {
        if (!isScientificNumeric(strTest))
        {
          if (m_ReportErrors)
          {
            fluxfilediff << "Diff at line " << numLine << " : compare numeric value with no numeric value (" << strRef << strRef << " != " << strTest << ")"
                         << std::endl;
        ...
        else
        {
              ...
              && (std::abs(vRef - vTest) > epsilon * vNorm)) // epsilon as relative error
          {
            if (m_ReportErrors)
            {
              fluxfilediff << "Diff at line " << numLine << " : std::abs ( (" << strRef << ") - (" << strTest << ") ) > " << epsilon << std::endl;
              differenceFoundInCurrentLine = true;
            }
            nbdiff++;
          }
        ...
        while (i < strRef.size())
        {
          ...
          else if ((etatCour == ETAT_CHAR) && (etatPrec == ETAT_NUM))
          {
            ...
            if ((strNumRef != strNumTest) && (std::abs(atof(strNumRef.c_str())) > m_EpsilonBoundaryChecking) &&
                (std::abs(atof(strNumRef.c_str()) - atof(strNumTest.c_str())) > epsilon * std::abs(atof(strNumRef.c_str())))) // epsilon as relative error
            {
              if (m_ReportErrors)
              {
                fluxfilediff << "Diff at line " << numLine << " : std::abs ( (" << strNumRef << ") - (" << strNumTest << ") ) > " << epsilon << std::endl;
                differenceFoundInCurrentLine = true;
              }
              nbdiff++;
            }
...
void TestHelper::ogrReportOnLayer(OGRLayer* ref_poLayer, const char* ref_pszWHERE, OGRGeometry* ref_poSpatialFilter, OGRLayer* test_poLayer,
                                  const char* test_pszWHERE, OGRGeometry* test_poSpatialFilter, int& nbdiff, double epsilon) const

{
  ...
  /// wkt representation.
  if ((ref_poLayer->GetSpatialRef() == nullptr) || (test_poLayer->GetSpatialRef() == nullptr) ||
      (!ref_poLayer->GetSpatialRef()->IsSame(test_poLayer->GetSpatialRef())))
  {
    ...
    if (test_poLayer->GetSpatialRef() == nullptr)
  ...
  if (ref_poDefn->GetFieldCount() == test_poDefn->GetFieldCount())
  {
    for (int iAttr = 0; iAttr < ref_poDefn->GetFieldCount(); ++iAttr)
    {
      ...
      otbCheckStringValue("Field GetFieldTypeName", ref_poField->GetFieldTypeName(ref_poField->GetType()),
                          test_poField->GetFieldTypeName(test_poField->GetType()), nbdiff, m_ReportErrors);
...
TestHelper::TestHelper() : m_ToleranceRatio(0), m_EpsilonBoundaryChecking(1.0e-30), m_ReportErrors(false), m_IgnoreLineOrder(false), m_MaxArea(1024 * 1024)
{
  m_SpecialTokens.push_back(std::pair<std::string, std::string>(std::string("Integer"), std::string("Integer64")));
}
